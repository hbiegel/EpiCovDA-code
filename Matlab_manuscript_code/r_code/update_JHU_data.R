...
JHU_folder <- "Dropbox/GitHub/COVID_19/csse_covid_19_data/csse_covid_19_daily_reports_us"
...
for (state_fips in all_fips){
  ...
  for (i in 1:num_days){
    ...
    currJHUdata <- read.csv(paste0(JHU_folder,"/",curr_filename))
  ...
  JHU_save_name <- 
    sprintf(
      "Dropbox/Research-Materials-Hannah/covid_related/HRB_COVID_code/state_hosp_data/JHU_state_%s.csv",
                           curr_abbrev)
